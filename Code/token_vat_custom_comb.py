...
#from bert_models_tavat import CustomTAVAT
...
def train(args,args2, train_dataset, model, tokenizer):
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            embeds_init=model.tavatbert.bert.embeddings.word_embeddings(input_ids)
            ...
            if args.adv_train == 0:
                ...
                for astep in range(args.adv_steps):
                    ...
                    delta_lb.requires_grad_()
                    delta_tok.requires_grad_()
                    #print("embeds_init Dimensions ",embeds_init.shape)
                    #print("delta_lb Dimensions ",delta_lb.shape)
                    #print("delta_tok Dimensions ",delta_tok.shape)
                    #parameter is [768]
                    #inputs_embeds = embeds_init + delta_lb + delta_tok

                    #inputs['inputs_embeds'] = inputs_embeds
                    trainbool=True
                    inputs,outputs= model(inputs, batch, embeds_init,    delta_lb, delta_tok,trainbool,args)
                    #print("thetasoft  ",model.theta.softmax(0))
                    #outputs = model(inputs)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0 or (
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if (
                        ...
                        if results['f1'] >= best_f1:
                            ...
                            model_to_save = (
                                model.tavatbert.module if hasattr(model.tavatbert, "module") else model.tavatbert)  
                            ...
                            #thetasoft=model.theta.softmax(0)
                            #theta_lb=[]
                            #vector=thetasoft[0,0,:,0].cpu().detach().numpy()
                            #logger.info("Max Theta for delta_lb LOOK %s", np.amax(vector))
                            #exit()
                            #theta_lb.append(np.amax(vector))
                            #theta_lb.append(np.amin(vector))

                            #theta_tok=[]
                            #vector2=thetasoft[1,0,:,0].cpu().detach().numpy()
                            #theta_tok.append(np.amax(vector2))
                            #theta_tok.append(np.amin(vector2))


                            #logger.info("Max Theta for delta_lb %s", theta_lb[0])
                            #logger.info("Min Theta for delta_lb %s", theta_lb[1])
                            #logger.info("Max Theta for delta_tok %s", theta_tok[0])
                            #logger.info("Min Theta for delta_tok %s", theta_tok[1])
                ...
                if True==False:#if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    if not os.path.exists(output_dir):
                        os.makedirs(output_dir)
                    model_to_save = (
                        model.tavatbert.module if hasattr(model, "module") else model.tavatbert
        ...
        if (args.evaluate_during_training):
            ...
            if results['f1'] >= best_f1:
                ...
                logger.info("Epoch is %s", epoch)
                model_to_save = (model.tavatbert.module if hasattr(model.tavatbert, "module") else model.tavatbert)  
                model_to_save.save_pretrained(output_dir)
                tokenizer.save_pretrained(output_dir)
                torch.save(args, os.path.join(output_dir, "training_args.bin"))
                logger.info("Saving model checkpoint to %s", output_dir)
...
def main():
    ...
    model2 = BertForTokenClassification.from_pretrained(output_dir)
    model2.to(args.device)
    pad_token_label_id = CrossEntropyLoss().ignore_index
    result, predictions = evaluate(args, args2, model2, tokenizer, pad_token_label_id ,test_dataset_regular,  mode="test", prefix = 'final')
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            if mode=='test':
                ...
                trainbool=False
                inputs,result= model(inputs, None, None,    None, None,trainbool,args)
