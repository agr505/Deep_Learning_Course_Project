...
def train(args,args2, train_dataset, model, tokenizer):
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            denorm = torch.norm(delta_tok.view(-1,delta_tok.size(-1)), p = 'nuc').view(-1, 1, 1)
            ...
            if args.adv_train == 0:
                ...
                for astep in range(args.adv_steps):
                    ...
                    denorm_lb = torch.norm(delta_lb_grad.view(bs, -1), p = 'nuc').view(-1, 1, 1)
                    denorm_lb = torch.clamp(denorm_lb, min=1e-8)
                    denorm_lb = denorm_lb.view(bs, 1, 1)


                    denorm_tok = torch.norm(delta_tok_grad, p = 'nuc') # B seq-len
                    ...
                    delta_norm_tok = torch.norm(delta_tok, p = 'nuc').detach()  # B seq-len
                    mean_norm_tok, _ = torch.max(delta_norm_tok, p = 'nuc', keepdim=True) # B,1
                    ...
                    delta_norm = torch.norm(total_delta.view(bs, -1).float(), p = 'nuc').detach()
